...
def run():
    ...
    #cur = conn.cursor(name='server_side_cursor', withhold=True) # Create a named server-side cursor
    dict_cur = conn.cursor(name='server_side_cursor', withhold=True, cursor_factory=psycopg2.extras.DictCursor) # Create a named server-side cursor
    ...
    for r in dict_cur:
        ...
        sql = """INSERT INTO grafana.device (%s) VALUES %s 
                ...
                , last_report = %s
                RETURNING id"""
        dbhelper.run_insert(cur2, sql, device, (device['interface'], device['error'], ts))
        dev_db_id = cur2.fetchone()[0]
        cur2.execute("""UPDATE grafana.device_inserter_state
                    SET var_value = %s
                    WHERE var_name = 'last_inserted_id' 
        ...
        if smart_attr_nvme:
            for k, v in smart_attr_nvme.items():
                ...
                dbhelper.run_insert(cur2, sql, device_smart_nvme)
        ...
        if device['interface'] == 'nvme' and 'nvme_smart_health_information_add_log' in rep:
            ...
            for k, v in dev_stats_parsed.items():
                ...
                dbhelper.run_insert(cur2, sql, device_smart_nvme_vs)
        ...
        if sata_smart_attr:
            for attr in sata_smart_attr.get('table', []):
                ...
                dbhelper.run_insert(cur2, sql, device_smart_sata)
